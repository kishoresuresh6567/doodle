...
let charm='eye',interaction='string',punchDoll='pirate',hitTool='fist',toolSwing=0,toolX=0,toolY=0,punchEnergy=0,punchSpin=0,punchAngle=0,punchVX=0,punchVY=0,punchX=0,punchY=0,punchHomeX=0,punchHomeY=0,punchHits=0,impactText='',impactLife=0,punchHeld=false,punchHold=0,punchPointerX=0,punchPointerY=0,lastPunchAt=0,punchCombo=0,comboLife=0,calmGlow=0,punchLaunched=false,punchOutTime=0;
let impactBits=[],oozeDrops=[];
const charmStorageKey='little-doodles.selected-charm';
const interactionStorageKey='little-doodles.interaction-mode';
const punchDollStorageKey='little-doodles.punch-doll';
const hitToolStorageKey='little-doodles.hit-tool';
...
function reset(){elasticLength=length;stretchVelocity=0;points=Array.from({length:N+1},(_,i)=>({x:anchor.x,y:anchor.y+length*i/N,px:anchor.x,py:anchor.y+length*i/N}));drag=null;angle=0;punchEnergy=punchSpin=punchAngle=punchVX=punchVY=punchX=punchY=punchHomeX=punchHomeY=impactLife=punchHits=punchHold=punchCombo=comboLife=calmGlow=lastPunchAt=punchOutTime=toolSwing=0;punchHeld=punchLaunched=false;impactBits=[];oozeDrops=[];document.querySelector('#hint').style.opacity=1;}
function physics(){
...
function draw(){ctx.clearRect(0,0,w,h);if(interaction==='punch'){drawPunchScene();return;}const p=points[N];const target=Math.atan2(p.x-anchor.x,p.y-anchor.y);angle+=(target-angle)*.09;const cx=p.x+Math.sin(angle)*radius*.88,cy=p.y+Math.cos(angle)*radius*.88;
...
ctx.restore();
const hitRadius=radius*(charm==='bubududu'?1.35:1);access.style.left=(cx-hitRadius)+'px';access.style.top=(cy-hitRadius)+'px';access.style.width=access.style.height=hitRadius*2+'px';canvas.dataset.charmX=cx;canvas.dataset.charmY=cy;canvas.dataset.hitRadius=hitRadius;
}
function updatePunch(){
 if(punchLaunched){punchX+=punchVX;punchY+=punchVY;punchVY+=.42;punchAngle+=punchSpin;punchSpin*=.995;punchEnergy*=.96;const launchedX=w*(w<700?.56:.67)+punchX,launchedY=h*.49+punchY;if(launchedX< -220||launchedX>w+220||launchedY>h+260||launchedY< -260){punchLaunched=false;punchOutTime=.9;document.querySelector('#status').textContent='Outta here! Take a breath…';}impactBits.forEach(bit=>{bit.x+=bit.vx;bit.y+=bit.vy;bit.vy+=.22;bit.life-=.025;});impactBits=impactBits.filter(bit=>bit.life>0);return;}
 if(punchOutTime>0){punchOutTime=Math.max(0,punchOutTime-dt);if(!punchOutTime){punchHomeX=punchHomeY=0;punchX=0;punchY=-h*.65;punchVX=0;punchVY=8;punchAngle=0;punchSpin=0;calmGlow=1;document.querySelector('#status').textContent='It came back. Ready when you are.';}return;}
 punchVX+=(punchHomeX-punchX)*.045;punchVY+=(punchHomeY-punchY)*.045;punchVX*=.91;punchVY*=.91;punchX+=punchVX;punchY+=punchVY;
 punchSpin+=-punchAngle*.055;punchSpin*=.89;punchAngle+=punchSpin;punchEnergy*=.9;toolSwing*=.86;impactLife=Math.max(0,impactLife-dt*1.7);comboLife=Math.max(0,comboLife-dt);calmGlow*=.965;if(!comboLife)punchCombo=0;
 if(punchHeld){punchHold=Math.min(1.4,punchHold+dt);punchVX*=.72;punchVY*=.72;punchSpin*=.72;}else punchHold=Math.max(0,punchHold-dt*5);
 impactBits.forEach(bit=>{bit.x+=bit.vx;bit.y+=bit.vy;bit.vy+=.22;bit.life-=.025;});impactBits=impactBits.filter(bit=>bit.life>0);
 oozeDrops.forEach(drop=>{drop.x+=drop.vx;drop.y+=drop.vy;drop.vy+=.1;drop.life-=.012;drop.r=Math.min(drop.r+ .035,8);});oozeDrops=oozeDrops.filter(drop=>drop.life>0);
}
function movePunchDoll(){punchHomeX=(Math.random()>.5?1:-1)*Math.min(w*.2,180);punchHomeY=(Math.random()-.5)*Math.min(h*.2,120);punchVX+=(punchHomeX-punchX)*.16;punchVY+=(punchHomeY-punchY)*.16;}
function oozePunch(x,y){for(let i=0;i<18;i++)oozeDrops.push({x:x+(Math.random()-.5)*28,y:y+(Math.random()-.5)*22,vx:(Math.random()-.5)*1.6,vy:.5+Math.random()*2.2,r:2+Math.random()*3,life:1.5+Math.random()});}
function useSelectedTool(x,y){toolSwing=1;toolX=x;toolY=y;if(hitTool==='knife'){oozePunch(x,y);punchAt(x,y,1.18,'SLICE!');}else if(hitTool==='screwdriver'){punchSpin+=(x<Number(canvas.dataset.charmX)?1:-1)*.2;punchAt(x,y,1.2,'TWIST!');}else if(hitTool==='hockey'){punchAt(x,y,1.48,'SLAPSHOT!');}else if(hitTool==='baseball'){punchAt(x,y,1.68,'HOME RUN!');}else punchAt(x,y);}
function drawHitTool(cx,cy,s){if(toolSwing<.025)return;ctx.save();ctx.globalAlpha=Math.min(1,toolSwing*2.5);ctx.translate(toolX+s*.7,toolY-s*.75);ctx.rotate(-1+(.95-toolSwing)*1.7);ctx.lineCap='round';ctx.lineJoin='round';if(hitTool==='fist'){ctx.translate(-s*.52,s*.72);ctx.scale(1.35-toolSwing*.2,1.1+toolSwing*.18);ctx.fillStyle='#d89b72';ctx.strokeStyle='#8f5d43';ctx.lineWidth=s*.018;ctx.beginPath();ctx.roundRect(-s*.19,-s*.08,s*.38,s*.34,s*.09);ctx.fill();ctx.stroke();for(let i=0;i<4;i++){ctx.beginPath();ctx.roundRect(-s*.21+i*s*.105,-s*.27,s*.12,s*.22,s*.055);ctx.fill();ctx.stroke();}ctx.beginPath();ctx.roundRect(s*.11,-s*.02,s*.18,s*.11,s*.05);ctx.fill();ctx.stroke();ctx.strokeStyle='#f3c84b';ctx.lineWidth=s*.025;for(let i=0;i<3;i++){const a=-.55+i*.55;ctx.beginPath();ctx.moveTo(-Math.cos(a)*s*.3,-Math.sin(a)*s*.3);ctx.lineTo(-Math.cos(a)*s*.5,-Math.sin(a)*s*.5);ctx.stroke();}}else if(hitTool==='knife'){ctx.fillStyle='#dbe2e3';ctx.strokeStyle='#718087';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(s*.12,-s*.82);ctx.lineTo(s*.28,-s*.08);ctx.closePath();ctx.fill();ctx.stroke();ctx.strokeStyle='#3d2b24';ctx.lineWidth=s*.13;ctx.beginPath();ctx.moveTo(s*.12,s*.03);ctx.lineTo(s*.04,s*.48);ctx.stroke();}else if(hitTool==='screwdriver'){ctx.strokeStyle='#c8d0d2';ctx.lineWidth=s*.055;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(0,-s*.82);ctx.stroke();ctx.strokeStyle='#d15a42';ctx.lineWidth=s*.19;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(0,s*.42);ctx.stroke();}else if(hitTool==='hockey'){ctx.strokeStyle='#774c2f';ctx.lineWidth=s*.11;ctx.beginPath();ctx.moveTo(0,s*.35);ctx.lineTo(0,-s*.88);ctx.stroke();ctx.lineWidth=s*.14;ctx.beginPath();ctx.moveTo(0,s*.34);ctx.lineTo(-s*.38,s*.45);ctx.stroke();}else if(hitTool==='baseball'){ctx.strokeStyle='#b77a3d';ctx.lineWidth=s*.16;ctx.beginPath();ctx.moveTo(0,s*.42);ctx.lineTo(0,-s*.76);ctx.stroke();ctx.strokeStyle='#84512b';ctx.lineWidth=s*.04;ctx.beginPath();ctx.moveTo(-s*.08,s*.3);ctx.lineTo(s*.08,s*.3);ctx.stroke();}ctx.restore();}
function punchAt(x,y,power=1,word=''){
 const cx=Number(canvas.dataset.charmX),cy=Number(canvas.dataset.charmY),dx=cx-x,dy=cy-y,d=Math.hypot(dx,dy)||1;
 punchVX+=(dx/d)*18*power+(Math.random()-.5)*5;punchVY+=(dy/d)*13*power-3*power;punchSpin+=(x<cx?1:-1)*.055*power;punchEnergy=Math.min(1.5,power);punchHits++;
 const words=['POW!','BONK!','BOP!','WHAM!','BOINK!'];impactText=word||words[punchHits%words.length];impactLife=Math.min(1.5,power);
 const colors=['#f3c84b','#ef6f61','#69a98b','#7898dc'];for(let i=0;i<Math.round(14*power);i++){const a=Math.random()*Math.PI*2,speed=(2+Math.random()*5)*Math.sqrt(power);impactBits.push({x,y,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed-1,life:.7+Math.random()*.35,color:colors[i%colors.length]});}
 const now=performance.now(),rapid=now-lastPunchAt<380;lastPunchAt=now;punchCombo=rapid?punchCombo+1:1;comboLife=1.1;
 if(power>=2.05){const direction=x<=cx?1:-1;punchLaunched=true;punchVX=direction*(40+power*7);punchVY=-25-power*4;punchSpin=direction*.19;impactText='KNOCKOUT!';impactLife=1.5;calmGlow=1.4;}
 if(!punchLaunched&&punchCombo>1&&punchCombo%8===0){calmGlow=1.2;impactText='RELEASE!';impactLife=1.4;for(let i=0;i<28;i++){const a=i*Math.PI/14,speed=3+Math.random()*4;impactBits.push({x:cx,y:cy,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,life:1,color:colors[i%colors.length]});}}
 const encouragement=['Nice. Let that tension go.','That was a good one. Breathe out.','Shake it off. You are doing great.'];document.querySelector('#status').textContent=punchLaunched?'KNOCKOUT! You sent it flying!':power>1?'BIG RELEASE! Take a slow breath out.':punchCombo%8===0?encouragement[(punchHits/8|0)%encouragement.length]:rapid?'Rapid-fire combo × '+punchCombo+'!':impactText+' Tap fast or hold to squeeze.';if(sound)chime();try{navigator.vibrate?.(power>1?[25,20,35]:16);}catch{}
}
function drawPunchScene(){
 const s=Math.min(w<700?w*.25:w*.15,h*.22,170),cx=w*(w<700?.56:.67)+punchX,cy=h*.49+punchY;
 if(punchOutTime>0){impactBits.forEach(bit=>{ctx.globalAlpha=Math.max(0,bit.life);circle(bit.x,bit.y,3+bit.life*3,bit.color);});ctx.globalAlpha=Math.min(1,punchOutTime*2);ctx.textAlign='center';ctx.font=`800 ${Math.round(Math.min(w,h)*.055)}px DM Sans`;ctx.fillStyle='#ef6f61';ctx.fillText('OUTTA HERE!',w/2,h*.46);ctx.font=`500 ${Math.round(Math.min(w,h)*.018)}px DM Sans`;ctx.fillStyle='#7b806f';ctx.fillText('breathe in… breathe out…',w/2,h*.52);ctx.globalAlpha=1;access.style.left='-9999px';return;}
 if(calmGlow>.01){const glow=ctx.createRadialGradient(cx,cy,s*.2,cx,cy,s*(2.2+calmGlow));glow.addColorStop(0,`rgba(105,169,139,${calmGlow*.12})`);glow.addColorStop(1,'rgba(105,169,139,0)');circle(cx,cy,s*(2.2+calmGlow),glow);}
 ctx.save();ctx.translate(cx+12,cy+s*1.5);ctx.scale(1,.2);const shadow=ctx.createRadialGradient(0,0,1,0,0,s);shadow.addColorStop(0,'#34291f35');shadow.addColorStop(1,'#34291f00');circle(0,0,s,shadow);ctx.restore();
 const squeeze=Math.min(.34,punchHold*.28);ctx.save();ctx.translate(cx,cy);ctx.rotate(punchAngle);ctx.scale(1+punchEnergy*.12+squeeze,1-punchEnergy*.09-squeeze);ctx.lineCap='round';ctx.lineJoin='round';
 const dollThemes={classic:{cloth:['#d7b882','#b78d58','#8e693f'],seam:'#765333',ear:'#c79262',left:'#405c62',right:'#8f4c49',patch:'#9270a5'},pirate:{cloth:['#d2ad78','#a7794c','#765033'],seam:'#503622',ear:'#bd8d5f',left:'#202631',right:'#8f4c49',patch:'#8d2635'},zombie:{cloth:['#a7b98a','#718260','#485744'],seam:'#344331',ear:'#8fa374',left:'#d7c04a',right:'#72569a',patch:'#66577c'},office:{cloth:['#d8b999','#b88a68','#856047'],seam:'#654734',ear:'#c99c7a',left:'#405c62',right:'#405c62',patch:'#47739b'},ninja:{cloth:['#5f6670','#343b46','#1d222b'],seam:'#111720',ear:'#4e5661',left:'#d9a838',right:'#d9a838',patch:'#9d2738'},cowboy:{cloth:['#d3a46e','#a66f40','#74492d'],seam:'#54321f',ear:'#bc8453',left:'#416b77',right:'#416b77',patch:'#a53932'},astronaut:{cloth:['#e8eceb','#b9c5c8','#7d8d94'],seam:'#536168',ear:'#c9d3d4',left:'#4f8fa8',right:'#4f8fa8',patch:'#d76a4a'},royal:{cloth:['#8064a8','#573c7d','#34254e'],seam:'#271b3d',ear:'#725795',left:'#e5bd45',right:'#e5bd45',patch:'#b33d5c'},punk:{cloth:['#5d6671','#333b46','#20252d'],seam:'#11161d',ear:'#4c5662',left:'#54b0a5',right:'#dd4c79',patch:'#d1b237'},chef:{cloth:['#eee6d7','#cec0ac','#9d8d79'],seam:'#716354',ear:'#d9caba',left:'#596f76',right:'#596f76',patch:'#c64b48'}};const dollTheme=dollThemes[punchDoll]||dollThemes.classic;
 const cloth=ctx.createLinearGradient(-s,-s,s,s);cloth.addColorStop(0,dollTheme.cloth[0]);cloth.addColorStop(.55,dollTheme.cloth[1]);cloth.addColorStop(1,dollTheme.cloth[2]);
 ctx.strokeStyle=dollTheme.seam;ctx.lineWidth=s*.24;ctx.beginPath();ctx.moveTo(-s*.48,s*.2);ctx.lineTo(-s*1.12,s*.65);ctx.moveTo(s*.48,s*.2);ctx.lineTo(s*1.12,s*.65);ctx.moveTo(-s*.3,s*1.02);ctx.lineTo(-s*.55,s*1.55);ctx.moveTo(s*.3,s*1.02);ctx.lineTo(s*.55,s*1.55);ctx.stroke();
 ctx.fillStyle=cloth;ctx.strokeStyle=dollTheme.seam;ctx.lineWidth=s*.035;ctx.beginPath();ctx.moveTo(-s*.66,-s*.05);ctx.quadraticCurveTo(-s*.74,s*.75,-s*.38,s*1.13);ctx.quadraticCurveTo(0,s*1.35,s*.38,s*1.13);ctx.quadraticCurveTo(s*.74,s*.75,s*.66,-s*.05);ctx.closePath();ctx.fill();ctx.stroke();
 ctx.beginPath();ctx.ellipse(0,-s*.62,s*.76,s*.78,0,0,Math.PI*2);ctx.fill();ctx.stroke();
 ctx.fillStyle=dollTheme.ear;ctx.beginPath();ctx.moveTo(-s*.55,-s*.78);ctx.quadraticCurveTo(-s*.82,-s*1.18,-s*.34,-s*1.27);ctx.lineTo(-s*.12,-s*1.3);ctx.lineTo(-s*.2,-s*.78);ctx.closePath();ctx.fill();ctx.stroke();
 ctx.strokeStyle='#ead6ad';ctx.lineWidth=s*.035;for(let i=-2;i<=2;i++){ctx.beginPath();ctx.moveTo(i*s*.2-s*.05,-s*1.27);ctx.lineTo(i*s*.2+s*.05,-s*1.12);ctx.stroke();}
 circle(-s*.28,-s*.72,s*.13,dollTheme.left);circle(s*.28,-s*.72,s*.13,dollTheme.right);ctx.strokeStyle='#efe3c8';ctx.lineWidth=s*.025;for(const ex of [-s*.28,s*.28]){ctx.beginPath();ctx.moveTo(ex-s*.08,-s*.8);ctx.lineTo(ex+s*.08,-s*.64);ctx.moveTo(ex+s*.08,-s*.8);ctx.lineTo(ex-s*.08,-s*.64);ctx.stroke();}
 if(punchDoll==='pirate'){
  ctx.fillStyle='#252a31';ctx.beginPath();ctx.moveTo(-s*.62,-s*1.18);ctx.quadraticCurveTo(0,-s*1.62,s*.62,-s*1.18);ctx.lineTo(s*.48,-s*.99);ctx.lineTo(-s*.48,-s*.99);ctx.closePath();ctx.fill();ctx.fillRect(-s*.78,-s*1.08,s*1.56,s*.13);
  ctx.strokeStyle='#29232a';ctx.lineWidth=s*.055;ctx.beginPath();ctx.moveTo(-s*.52,-s*.94);ctx.lineTo(s*.3,-s*.67);ctx.stroke();circle(s*.28,-s*.72,s*.16,'#29232a');ctx.fillStyle='#f0dfb8';ctx.font=`700 ${s*.2}px serif`;ctx.textAlign='center';ctx.fillText('☠',0,-s*1.13);
 }else if(punchDoll==='zombie'){
  ctx.fillStyle='#536548';ctx.beginPath();ctx.moveTo(s*.48,-s*1.08);ctx.lineTo(s*.73,-s*.88);ctx.lineTo(s*.5,-s*.68);ctx.closePath();ctx.fill();ctx.strokeStyle='#d7d1ae';ctx.lineWidth=s*.025;for(let i=0;i<4;i++){ctx.beginPath();ctx.moveTo(s*(.38+i*.07),-s*(1.05-i*.05));ctx.lineTo(s*(.5+i*.06),-s*(.96-i*.05));ctx.stroke();}ctx.strokeStyle='#344331';ctx.lineWidth=s*.035;ctx.beginPath();ctx.moveTo(-s*.48,-s*.45);ctx.lineTo(-s*.2,-s*.57);for(let i=0;i<4;i++){const x=-s*.43+i*s*.08;ctx.moveTo(x,-s*.49);ctx.lineTo(x+s*.03,-s*.58);}ctx.stroke();
 }else if(punchDoll==='office'){
  ctx.strokeStyle='#3d4652';ctx.lineWidth=s*.035;ctx.strokeRect(-s*.49,-s*.87,s*.38,s*.28);ctx.strokeRect(s*.11,-s*.87,s*.38,s*.28);ctx.beginPath();ctx.moveTo(-s*.11,-s*.73);ctx.lineTo(s*.11,-s*.73);ctx.stroke();ctx.fillStyle='#eee5d2';ctx.beginPath();ctx.moveTo(-s*.32,s*.05);ctx.lineTo(0,s*.3);ctx.lineTo(s*.32,s*.05);ctx.lineTo(s*.48,s*.45);ctx.lineTo(-s*.48,s*.45);ctx.closePath();ctx.fill();ctx.fillStyle='#9b3945';ctx.beginPath();ctx.moveTo(-s*.08,s*.18);ctx.lineTo(s*.08,s*.18);ctx.lineTo(s*.13,s*.72);ctx.lineTo(0,s*.86);ctx.lineTo(-s*.13,s*.72);ctx.closePath();ctx.fill();ctx.fillStyle='#fff';ctx.fillRect(s*.22,s*.3,s*.34,s*.18);ctx.fillStyle='#555';ctx.font=`600 ${s*.07}px DM Sans`;ctx.textAlign='center';ctx.fillText('DEADLINE',s*.39,s*.42);
 }else if(punchDoll==='ninja'){
  ctx.fillStyle='#171c24';ctx.fillRect(-s*.72,-s*1.02,s*1.44,s*.52);ctx.fillStyle='#b63845';ctx.fillRect(-s*.75,-s*1.04,s*1.5,s*.1);ctx.beginPath();ctx.moveTo(s*.7,-s*.99);ctx.lineTo(s*1.08,-s*.8);ctx.lineTo(s*.76,-s*.72);ctx.closePath();ctx.fill();ctx.fillStyle='#e7b54a';ctx.beginPath();ctx.moveTo(-s*.45,-s*.8);ctx.lineTo(-s*.08,-s*.72);ctx.lineTo(-s*.43,-s*.63);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(s*.45,-s*.8);ctx.lineTo(s*.08,-s*.72);ctx.lineTo(s*.43,-s*.63);ctx.closePath();ctx.fill();
 }else if(punchDoll==='cowboy'){
  ctx.fillStyle='#75451f';ctx.beginPath();ctx.moveTo(-s*.48,-s*1.25);ctx.quadraticCurveTo(0,-s*1.47,s*.48,-s*1.25);ctx.lineTo(s*.4,-s*.98);ctx.lineTo(-s*.4,-s*.98);ctx.closePath();ctx.fill();ctx.fillRect(-s*.82,-s*1.05,s*1.64,s*.14);ctx.fillStyle='#a83235';ctx.beginPath();ctx.moveTo(-s*.45,-s*.42);ctx.lineTo(s*.45,-s*.42);ctx.lineTo(0,-s*.12);ctx.closePath();ctx.fill();ctx.fillStyle='#e3bb46';heroStar(s*.42,s*.28,s*.15,'#e3bb46');
 }else if(punchDoll==='astronaut'){
  ctx.fillStyle='#91aab4aa';ctx.strokeStyle='#eef5f4';ctx.lineWidth=s*.075;ctx.beginPath();ctx.ellipse(0,-s*.7,s*.82,s*.9,0,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle='#334854';ctx.fillRect(-s*.5,s*.05,s, s*.67);ctx.fillStyle='#84d1ca';circle(-s*.25,s*.3,s*.07,'#84d1ca');circle(0,s*.3,s*.07,'#e6b74a');circle(s*.25,s*.3,s*.07,'#d75e64');ctx.strokeStyle='#eef5f4';ctx.lineWidth=s*.045;ctx.strokeRect(-s*.48,s*.03,s*.96,s*.72);
 }else if(punchDoll==='royal'){
  ctx.fillStyle='#e5bd45';ctx.beginPath();ctx.moveTo(-s*.55,-s*1.12);ctx.lineTo(-s*.48,-s*1.55);ctx.lineTo(-s*.16,-s*1.25);ctx.lineTo(0,-s*1.62);ctx.lineTo(s*.16,-s*1.25);ctx.lineTo(s*.48,-s*1.55);ctx.lineTo(s*.55,-s*1.12);ctx.closePath();ctx.fill();ctx.fillStyle='#b52f54';circle(0,-s*1.32,s*.08,'#b52f54');ctx.fillStyle='#efe0b4';ctx.beginPath();ctx.moveTo(-s*.55,s*.02);ctx.lineTo(0,s*.35);ctx.lineTo(s*.55,s*.02);ctx.lineTo(s*.7,s*.9);ctx.lineTo(-s*.7,s*.9);ctx.closePath();ctx.fill();circle(0,s*.43,s*.16,'#e5bd45');
 }else if(punchDoll==='punk'){
  const spikes=[[-.48,-1.1],[-.3,-1.5],[-.1,-1.14],[0,-1.62],[.12,-1.13],[.34,-1.5],[.5,-1.08]];ctx.fillStyle='#dd4c79';ctx.beginPath();spikes.forEach(([x,y],i)=>i?ctx.lineTo(s*x,s*y):ctx.moveTo(s*x,s*y));ctx.lineTo(s*.45,-s*.92);ctx.lineTo(-s*.45,-s*.92);ctx.closePath();ctx.fill();ctx.fillStyle='#20252d';ctx.beginPath();ctx.moveTo(-s*.6,s*.05);ctx.lineTo(0,s*.35);ctx.lineTo(s*.6,s*.05);ctx.lineTo(s*.7,s*.9);ctx.lineTo(-s*.7,s*.9);ctx.closePath();ctx.fill();for(const side of [-1,1])circle(side*s*.42,s*.35,s*.045,'#d1b237');
 }else if(punchDoll==='chef'){
  ctx.fillStyle='#f7f2e8';circle(-s*.34,-s*1.31,s*.3,'#f7f2e8');circle(0,-s*1.43,s*.36,'#f7f2e8');circle(s*.34,-s*1.31,s*.3,'#f7f2e8');ctx.fillRect(-s*.58,-s*1.32,s*1.16,s*.31);ctx.strokeStyle='#887b6c';ctx.lineWidth=s*.025;for(const [hx,hy,hr] of [[-.34,-1.31,.3],[0,-1.43,.36],[.34,-1.31,.3]]){ctx.beginPath();ctx.arc(s*hx,s*hy,s*hr,0,Math.PI*2);ctx.stroke();}ctx.strokeRect(-s*.58,-s*1.32,s*1.16,s*.31);ctx.strokeStyle='#c9bbaa';ctx.lineWidth=s*.018;ctx.beginPath();ctx.moveTo(-s*.38,-s*1.12);ctx.lineTo(s*.38,-s*1.12);ctx.stroke();ctx.fillStyle='#faf7ef';ctx.beginPath();ctx.moveTo(-s*.55,s*.04);ctx.lineTo(s*.55,s*.04);ctx.lineTo(s*.66,s*.9);ctx.lineTo(-s*.66,s*.9);ctx.closePath();ctx.fill();ctx.strokeStyle='#c64b48';ctx.lineWidth=s*.035;ctx.stroke();ctx.fillStyle='#c64b48';ctx.font=`700 ${s*.11}px DM Sans`;ctx.textAlign='center';ctx.fillText('CHEF',0,s*.48);
 }
 ctx.strokeStyle='#5f412d';ctx.lineWidth=s*.045;ctx.beginPath();if(punchEnergy>.25){ctx.moveTo(-s*.2,-s*.35);ctx.quadraticCurveTo(0,-s*.18,s*.2,-s*.35);}else{ctx.moveTo(-s*.2,-s*.28);ctx.quadraticCurveTo(0,-s*.42,s*.2,-s*.28);}ctx.stroke();
 ctx.strokeStyle='#6e4b30';ctx.lineWidth=s*.028;for(const side of [-1,1]){ctx.beginPath();ctx.moveTo(side*s*.52,s*.03);ctx.lineTo(side*s*.66,s*.2);ctx.moveTo(side*s*.49,s*.16);ctx.lineTo(side*s*.65,s*.03);ctx.stroke();}
 ctx.restore();
 drawHitTool(cx,cy,s);
 oozeDrops.forEach(drop=>{ctx.globalAlpha=Math.min(1,drop.life);ctx.fillStyle='#9e2434';ctx.beginPath();ctx.ellipse(drop.x,drop.y,drop.r,drop.r*1.65,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#d85159';circle(drop.x-drop.r*.25,drop.y-drop.r*.45,drop.r*.28,'#d85159');});ctx.globalAlpha=1;
 impactBits.forEach(bit=>{ctx.globalAlpha=Math.max(0,bit.life);circle(bit.x,bit.y,3+bit.life*3,bit.color);});ctx.globalAlpha=1;
 if(impactLife>0){ctx.save();ctx.translate(cx-s*.85,cy-s*.8);ctx.rotate(-.18);ctx.globalAlpha=Math.min(1,impactLife*2);ctx.font=`800 ${Math.round(s*.28)}px DM Sans`;ctx.fillStyle='#ef6f61';ctx.strokeStyle='#fff8e8';ctx.lineWidth=5;ctx.strokeText(impactText,0,0);ctx.fillText(impactText,0,0);ctx.restore();}
 if(punchCombo>1&&comboLife>0){ctx.save();ctx.globalAlpha=Math.min(1,comboLife*2);ctx.textAlign='center';ctx.font=`700 ${Math.round(s*.13)}px DM Sans`;ctx.fillStyle='#7b806f';ctx.fillText(`COMBO × ${punchCombo}`,cx,cy-s*1.58);ctx.restore();}
 if(punchHeld){const progress=Math.min(1,punchHold/1.05),mw=Math.min(s*2.2,w*.72),mh=18,mx=cx-mw/2,my=Math.min(h-112,cy+s*1.72);ctx.fillStyle='#d9d4c8';ctx.fillRect(mx,my,mw,mh);const meterColor=progress<.25?'#69a98b':progress<.58?'#e0a83e':progress<.86?'#cf5b5f':'#8e2940';ctx.fillStyle=meterColor;ctx.fillRect(mx,my,mw*progress,mh);ctx.strokeStyle='#f7f5ef';ctx.lineWidth=2;for(const mark of [.25,.58,.86]){ctx.beginPath();ctx.moveTo(mx+mw*mark,my);ctx.lineTo(mx+mw*mark,my+mh);ctx.stroke();}ctx.textAlign='center';ctx.font=`700 ${Math.max(9,Math.round(s*.075))}px DM Sans`;ctx.fillStyle='#7b806f';ctx.fillText('PUNCH',mx+mw*.125,my+mh+15);ctx.fillText('MOVE',mx+mw*.415,my+mh+15);ctx.fillText('OOZE',mx+mw*.72,my+mh+15);ctx.fillText('THROW OUT',mx+mw*.93,my+mh+15);}
 const hitRadius=s*1.35;canvas.dataset.charmX=cx;canvas.dataset.charmY=cy;canvas.dataset.hitRadius=hitRadius;access.style.left=(cx-hitRadius)+'px';access.style.top=(cy-hitRadius)+'px';access.style.width=access.style.height=hitRadius*2+'px';
}
function frame(t){accumulator+=Math.min((t-last)/1000,.04);last=t;if(!paused){while(accumulator>=dt){interaction==='punch'?updatePunch():physics();accumulator-=dt;}}else accumulator=0;draw();requestAnimationFrame(frame);}
function hit(e){return Math.hypot(e.clientX-Number(canvas.dataset.charmX),e.clientY-Number(canvas.dataset.charmY))<Number(canvas.dataset.hitRadius||radius)+15;}
canvas.addEventListener('pointerdown',e=>{if(!hit(e))return;if(paused)togglePause();document.querySelector('#hint').style.opacity=0;if(interaction==='punch'){canvas.setPointerCapture(e.pointerId);punchHeld=true;punchHold=0;punchPointerX=e.clientX;punchPointerY=e.clientY;canvas.style.cursor='grabbing';document.querySelector('#status').textContent='Keep holding to squeeze…';return;}canvas.setPointerCapture(e.pointerId);drag={x:points[N].x,y:points[N].y,ox:e.clientX-points[N].x,oy:e.clientY-points[N].y};canvas.style.cursor='grabbing';document.querySelector('#status').textContent='Hold on. Let go. Find your flow.';});
canvas.addEventListener('pointermove',e=>{if(interaction==='punch'&&punchHeld){punchPointerX=e.clientX;punchPointerY=e.clientY;document.querySelector('#status').textContent=punchHold>=1.05?'THROW OUT ready — release!':punchHold>=.62?'OOZE stage — release now!':punchHold>=.27?'MOVE stage — release to shove it!':'Keep holding to charge the meter…';return;}if(drag){let dx=e.clientX-drag.ox-anchor.x,dy=e.clientY-drag.oy-anchor.y;const distance=Math.hypot(dx,dy),max=length*2.5;if(distance>max){dx*=max/distance;dy*=max/distance;}drag.x=anchor.x+dx;drag.y=anchor.y+dy;}else canvas.style.cursor=hit(e)?(interaction==='punch'?'pointer':'grab'):'default';});
function release(e){if(interaction==='punch'&&punchHeld){const held=punchHold,x=e?.clientX||punchPointerX,y=e?.clientY||punchPointerY;punchHeld=false;canvas.style.cursor='pointer';if(held>=1.05)punchAt(x,y,2.35,'KNOCKOUT!');else if(held>=.62){oozePunch(x,y);punchAt(x,y,1.85,'SPLAT!');document.querySelector('#status').textContent='OOZE HIT! Let that pressure out.';}else if(held>=.27){movePunchDoll();punchAt(x,y,1.3,'SHOVE!');document.querySelector('#status').textContent='SHOVED! The doll moved to a new spot.';}else useSelectedTool(x,y);return;}if(!drag)return;drag=null;canvas.style.cursor='grab';if(sound)chime();document.querySelector('#status').textContent='A little physics. A little magic.';}
...
access.addEventListener('click',()=>{if(interaction==='punch')useSelectedTool(Number(canvas.dataset.charmX),Number(canvas.dataset.charmY));});
...
const punchDoodle=document.querySelector('#punch-doodle');
punchDoodle.setAttribute('aria-expanded','false');punchDoodle.setAttribute('aria-controls','punch-options');punchDoodle.lastElementChild.className='menu-chevron';punchDoodle.lastElementChild.innerHTML='&#8964;';
punchDoodle.insertAdjacentHTML('afterend','<div id="punch-options" class="punch-options" hidden role="group" aria-label="Choose a themed voodoo doll"><button data-punch-doll="classic" aria-pressed="false">Classic Voodoo</button><button data-punch-doll="pirate" aria-pressed="true">Pirate Voodoo</button><button data-punch-doll="zombie" aria-pressed="false">Zombie Voodoo</button><button data-punch-doll="office" aria-pressed="false">Office Stress Voodoo</button><button data-punch-doll="ninja" aria-pressed="false">Ninja Voodoo</button><button data-punch-doll="cowboy" aria-pressed="false">Cowboy Voodoo</button><button data-punch-doll="astronaut" aria-pressed="false">Astronaut Voodoo</button><button data-punch-doll="royal" aria-pressed="false">Royal Voodoo</button><button data-punch-doll="punk" aria-pressed="false">Punk Voodoo</button><button data-punch-doll="chef" aria-pressed="false">Chef Voodoo</button></div>');
const punchOptions=document.querySelector('#punch-options');
document.body.insertAdjacentHTML('beforeend','<div id="punch-toolbox" class="punch-toolbox" hidden role="group" aria-label="Choose a hitting tool"><span>HIT WITH</span><button data-hit-tool="fist" aria-pressed="true">Fist</button><button data-hit-tool="knife" aria-pressed="false">Knife</button><button data-hit-tool="screwdriver" aria-pressed="false">Screwdriver</button><button data-hit-tool="hockey" aria-pressed="false">Hockey Stick</button><button data-hit-tool="baseball" aria-pressed="false">Baseball Bat</button></div>');
const punchToolbox=document.querySelector('#punch-toolbox');
...
stringToggle.addEventListener('click',()=>{
 interaction='string';
 try{localStorage.setItem(interactionStorageKey,interaction);}catch{}
 selectCharm(charm);
 punchDoodle.setAttribute('aria-pressed','false');
 punchDoodle.setAttribute('aria-expanded','false');punchOptions.hidden=true;
 punchToolbox.hidden=true;
 heroTitle.innerHTML='A little<br>good energy<span>.</span>';
 heroCopy.innerHTML='Pick a charm. Give it a pull.<br>Watch it bounce and swing.';
 document.querySelector('#hint span').textContent='go on, give it a tug';
 stringOptions.hidden=!stringOptions.hidden;
 stringToggle.setAttribute('aria-expanded',String(!stringOptions.hidden));
});
document.querySelectorAll('[data-charm]').forEach(button=>button.addEventListener('click',()=>{interaction='string';try{localStorage.setItem(interactionStorageKey,interaction);}catch{}punchToolbox.hidden=true;punchDoodle.setAttribute('aria-pressed','false');heroTitle.innerHTML='A little<br>good energy<span>.</span>';heroCopy.innerHTML='Pick a charm. Give it a pull.<br>Watch it bounce and swing.';document.querySelector('#hint span').textContent='go on, give it a tug';closeDoodleMenu(true);}));
function activatePunchMode(closeMenu=false){interaction='punch';try{localStorage.setItem(interactionStorageKey,interaction);localStorage.setItem(punchDollStorageKey,punchDoll);localStorage.setItem(hitToolStorageKey,hitTool);}catch{}reset();punchToolbox.hidden=false;document.querySelectorAll('[data-punch-doll]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.punchDoll===punchDoll)));document.querySelectorAll('[data-hit-tool]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.hitTool===hitTool)));stringOptions.hidden=true;stringToggle.setAttribute('aria-expanded','false');heroTitle.innerHTML='Let your<br>Frustrations out<span>.</span>';heroCopy.innerHTML='A soft target for heavy feelings.<br>Hit it. Squeeze it. Let it go.';canvas.style.cursor='pointer';canvas.setAttribute('aria-label','A large interactive '+punchDoll+' punch doll with four squeeze stages');access.setAttribute('aria-label','Use the '+hitTool+' on the doodle. Hold to choose punch, move, ooze, or throw out.');document.querySelector('#hint span').textContent='hold · choose a stage · release';document.querySelector('#status').textContent='Selected tool: '+(hitTool==='fist'?'your fist':hitTool)+'. Tap the doll to use it.';if(closeMenu){punchOptions.hidden=true;punchDoodle.setAttribute('aria-expanded','false');closeDoodleMenu(true);}}
punchDoodle.addEventListener('click',()=>{const open=punchOptions.hidden;punchOptions.hidden=!open;punchDoodle.setAttribute('aria-expanded',String(open));activatePunchMode();});
document.querySelectorAll('[data-punch-doll]').forEach(button=>button.addEventListener('click',()=>{punchDoll=button.dataset.punchDoll;activatePunchMode(true);}));
document.querySelectorAll('[data-hit-tool]').forEach(button=>button.addEventListener('click',()=>{hitTool=button.dataset.hitTool;try{localStorage.setItem(hitToolStorageKey,hitTool);}catch{}canvas.style.cursor='pointer';document.querySelectorAll('[data-hit-tool]').forEach(tool=>tool.setAttribute('aria-pressed',String(tool===button)));access.setAttribute('aria-label','Use the '+hitTool+' on the doodle.');document.querySelector('#status').textContent='Selected tool: '+(hitTool==='fist'?'your fist':hitTool)+'. Tap the doll to use it.';}));
...
try{
 ...
 const savedPunchDoll=localStorage.getItem(punchDollStorageKey);
 if(['classic','pirate','zombie','office','ninja','cowboy','astronaut','royal','punk','chef'].includes(savedPunchDoll))punchDoll=savedPunchDoll;
 const savedHitTool=localStorage.getItem(hitToolStorageKey);
 if(['fist','knife','screwdriver','hockey','baseball'].includes(savedHitTool))hitTool=savedHitTool;
 if(localStorage.getItem(interactionStorageKey)==='punch')activatePunchMode();
}catch{}
